package lambda.customized.type.helperlike;
...
public class CustomizedHelperLikeType {
	static CustomizedListLikeType<? extends Object> Nil = new CustomizedListLikeType<Object>() {
		public Object head() {
			return null;
		}
	...
	public static <T> CustomizedListLikeType<T> nil() {
		return (CustomizedListLikeType<T>) Nil;
	}

	public static <T> CustomizedListLikeType<T> cons(final T x, final CustomizedListLikeType<T> xs) {
	...
	public static <T> CustomizedListLikeType<T> list(T... elems) {
		if (elems.length == 0)
			return nil();
	...
	public static <T, R> CustomizedListLikeType<R> map(CustomizedListLikeType<T> lt, CustomizedF1<T, R> f) {
		if(lt == Nil) return nil();
		return cons(f.apply(lt.head()), map(lt.tail(), f));
	}

	public static <T> CustomizedListLikeType<T> filter(CustomizedListLikeType<T> lt, CustomizedF1<T, Boolean> f) {
	...
	public static <T, R> R reduce(CustomizedListLikeType<T> lt, CustomizedF2<T, R> f, R r) {
	if(lt == Nil) return r;
